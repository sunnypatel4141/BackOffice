 ...
 * @author Sunny Patel
 */
public class MiscPayout extends StoreInfo implements ActionListener {
       
    JDialog miscDialog = new JDialog(frame, "Misc Payout Buttons", true);
    JButton deleteItem, addItem, modifyItem, closeDialog;
    
    String cn[] = {"Product ID", "Button ID", "Button Name"};
    Object data[][] = null;
    private DefaultTableModel modelpayout = new DefaultTableModel(data, cn);
    JTable table = new JTable(modelpayout);
    ...
    private void loadData() {
        try {
            ...
            while(rs.next()) {
                ...
                modelpayout.addRow(row);
            }
        } catch (SQLException ex) {
            Logger.getLogger(MiscPayout.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(trigger == deleteItem) {
            ...
            MiscPayoutEditor mnte = new MiscPayoutEditor();
            reloadTable();
            ...
            String buttonID = modelpayout.getValueAt(selectedRow, 0).toString();
            MiscPayoutEditor mnte = new MiscPayoutEditor(buttonID);
            reloadTable();
    ...
    /**
     * Delete the given buttons from the payout table
     * also the prid of the given row is set to discontinued
     */
    private void deleteButton(int selectedRow) {
        try {
            String prid = modelpayout.getValueAt(selectedRow, 0).toString();
            String id = modelpayout.getValueAt(selectedRow, 0).toString();
            // Get the details of the current row
            String sql = "delete from payout where id='" + id + "'";
    ...
    private void reloadTable() {
        int row = modelpayout.getRowCount();
        while(row > 0) {
            row--;
            modelpayout.removeRow(0);
        }
        loadData();
    }
