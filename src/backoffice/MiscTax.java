 ...
 * @author Sunny Patel
 */
class MiscTax extends StoreInfo implements ActionListener {
    
    JDialog miscDialog = new JDialog(frame, "Misc Tax Buttons", true);
    JButton deleteItem, addItem, modifyItem, closeDialog;
    
    String cn[] = {"Product ID", "Button ID", "Button Name"};
    Object data[][] = null;
    private DefaultTableModel modeltax = new DefaultTableModel(data, cn);
    JTable table = new JTable(modeltax);
    ...
    private void loadData() {
        try {
            ...
            Logger.getLogger(MiscTax.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        if(trigger == deleteItem) {
            ...
            int confirm = JOptionPane.showConfirmDialog(miscDialog, 
            ...
            MiscTaxEditor mnte = new MiscTaxEditor();
            reloadTable();
            ...
            String buttonID = modeltax.getValueAt(selectedRow, 0).toString();
            MiscTaxEditor mnte = new MiscTaxEditor(buttonID);
            reloadTable();
    ...
    /**
     * Delete the given buttons from the tax table
     * also the prid of the given row is set to discontinued
     */
    private void deleteButton(int selectedRow) {
        try {
            String prid = modeltax.getValueAt(selectedRow, 0).toString();
            String id = modeltax.getValueAt(selectedRow, 0).toString();
            // Get the details of the current row
            String sql = "delete from tax where id='" + id + "'";
    ...
    private void reloadTable() {
        int row = modeltax.getRowCount();
        while(row > 0) {
            row--;
            modeltax.removeRow(0);
        }
        loadData();
    }
